...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.cluster_feature_embed = nn.Linear(self.concat_size, self.hidden_size)
        # self.msvd_msrvtt_concept1000_train = h5py.File('data/MSVD/msvd&msrvtt_concept1000_feat_train.h5','r')['concept_features'][:,:]
        # self.register_buffer('msvd_msrvtt_concept1000_feat_train', torch.from_numpy(self.msvd_msrvtt_concept1000_train).float())
        ...
        if opt.dataset == 'msvd':
            ...
            self.msvd_concept1000_train = h5py.File('data/MSVD/msvd_concept1000_feat_val.h5','r')['concept_features'][:,:]
            # self.msvd_concept3000_train = h5py.File('data/MSVD/msvd_concept3000_feat_train.h5','r')['concept_features'][:,:]
            ...
            # self.register_parameter("learnable_concept",nn.Parameter(torch.from_numpy(self.msvd_concept1000_train).float()))
            self.register_parameter("learnable_concept",nn.Parameter(torch.zeros(1, 1000, self.hidden_size)))
            ...
            self.msrvtt_concept1000_train = h5py.File('data/MSRVTT/msrvtt_concept1000_feat_val.h5','r')['concept_features'][:,:]
            # self.msrvtt_concept3000_train = h5py.File('data/MSRVTT/msrvtt_concept3000_feat_train.h5','r')['concept_features'][:,:]
            ...
            # self.register_parameter("learnable_concept",nn.Parameter(torch.from_numpy(self.msrvtt_concept1000_train).float()))
            # self.register_parameter("learnable_concept",nn.Parameter(torch.zeros_like(self.concept1000_feat_train).float()))

        elif opt.dataset == 'vatex':
            print("using {}".format('vatex_concept_feat_train.h5'))
            self.vatex_concept1000_train = h5py.File('data/VATEX/vatex_concept1000_feat_trainval.h5','r')['concept_features'][:,:]
            self.register_buffer('concept1000_feat_train', torch.from_numpy(self.vatex_concept1000_train).float())

        # self.msvd_concept_all_train = h5py.File('data/three_dataset_concept1000_feat_train.h5','r')['concept_features'][:,:]
        # self.register_buffer('conceptall_train', torch.from_numpy(self.msvd_concept_all_train).float())

    def _init_weights(self):
        nn.init.xavier_normal_(self.frame_feature_embed.weight)
        nn.init.xavier_normal_(self.cluster_feature_embed.weight)
        nn.init.xavier_normal_(self.learnable_concept.weight)
        nn.init.constant_(self.frame_feature_embed.bias, 0)
        nn.init.constant_(self.cluster_feature_embed.bias, 0)
    ...
    def forward(self, cnn_feats):
        ...
        frame_feats = self.transformer_encoder(frame_feats)

        # cluster_feats = self.cluster_feature_embed(self.learnable_concept)
        cluster_feats = self.learnable_concept
        _, _, cnn_feats_attention3 = frame_feats, frame_feats, frame_feats
        ...
        return frame_feats, cnn_feats_attention3