...
def convert_to_one_hot_topk(indices, num_classes):
    """
    Args:
        indices (tensor): A vector containing indices,
            whose size is (batch_size,).
...
def greedy_select_topk(logits, topk = 10, mask=None):
    probs = masked_softmax(logits=logits, mask=mask)
    one_hot = convert_to_one_hot_topk(indices= torch.topk(probs, topk, 1)[1],
                                 num_classes=logits.size(1))
    return one_hot
...
def st_gumbel_softmax_topk(logits, temperature=1.0, topk = 10, mask=None):
    """
    Return the result of Straight-Through Gumbel-Softmax Estimation.
    It approximates the discrete sampling via Gumbel-Softmax trick
    and applies the biased ST estimator.
    ...
    y = logits + gumbel_noise
    y = masked_softmax(logits=y / temperature, mask=mask)
    y_argmax = torch.topk(y, topk, dim=1)[1]
    y_hard = convert_to_one_hot_topk(indices=y_argmax, num_classes=y.size(1)).float()
    y = (y_hard - y).detach() + y
    return y
